...
export function pianorollToSequence(
  ...
  for (let s = 0; s < numberOfSteps; s++) {
    for (let p = 0; p < NUM_PITCHES; p++) {
      ...
      for (let v = 0; v < NUM_OUTPUT_VOICES; v++) {
...
export function sequenceToPianoroll(
  ...
  notes.forEach(note => {
    ...
    let pitchIndex = note.pitch - MIN_PITCH;
    const stepIndex = note.quantizedStartStep;
    //temproray fix. 
    if (pitchIndex >= NUM_PITCHES) {
      logging.log(
        `Notes pitch is ${pitchIndex} greater than max ${NUM_PITCHES-1}. 
        Clipping this note pitch to ${MAX_PITCH}.`,'Coconet',
        logging.Level.WARN);
        pitchIndex = NUM_PITCHES-1;
        note.pitch = 77;
    }
    if (pitchIndex < 0) {
      logging.log(
        `Notes pitch is ${pitchIndex} because note pitch less than 
        ${MIN_PITCH}, setting it to zero and clipping this note pitch to ${MIN_PITCH}.`,'Coconet',
        logging.Level.WARN);
        pitchIndex = 0;
        note.pitch = 44;
    }
